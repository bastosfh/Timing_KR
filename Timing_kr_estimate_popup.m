% Este script (teoricamente) roda todas as condições da MT


cd '~/Timing_CR';
% Timing_cr_mouse_estimativa_popup_flavio

% Notas da versão no final do arquivo

% Close all screens before start
sca;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% configuracoes %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Nome do experimento. Incluir o nome do grupo no codigo11


experimento = 'MT_Auto_estima';
% det_auto_CR
% det_auto_boas
% det_pos_100
% det_pos_33
...
ntt = 70;
% Número (sem aspas simples)
% Aquisição = 70 - 123
% Transferência = 20 (será definido automaticamente pelo número de linhas em 'transfer_vels_2.txt' ou 'transfer_vels_3.txt')
...
% 'rtc' = teste de retenção. Similar à condição trf, porém, exibe o questionário antes da primeira tentativa (não ocorre ao rodar 'trf')
...
arquivo_frq_feedback = 'arquivo_frq_feedback_33.txt';
...
% Determina se a tela com a palavra "Estimativa..." será apresentada durante a fase de aquisição.
% Mesmo que a tela não seja apresentada o popup continua sendo exibido (para digitação da "estimativa" 99999)
estimativa_durante_aq = '1';
% '1' = Apresenta "Estimativa..." na tela
% '2' = Não apresenta e mostra a pergunta (auto) se deseja ou não CR
...
% Número de tentativas até o baseline
ntt_baseline = 23;
...
%%%%%%%%%%%%%% enderecos fisicos %%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
script_path = '/home/coleta3/Timing_CR';
interval_to_target_mov_path = '/home/coleta3/Timing_CR/interval_to_target_mov.txt';
line_positions_path = '/home/coleta3/Timing_CR/line_positions.txt';
myu3_path = '/home/coleta3/LabJackPython/labjack-LJFuse/root-ljfuse/MyU3/connection/AIN0'; %define o caminho para ler a resposta do switch

txt_name = sprintf('%s_suj%s.txt', experimento, sjnum); %nome do arquivo txt para salvar todas as tentativas do sujeito
arquivo_sj_path = sprintf('/home/coleta3/Timing_CR/%s/%s_suj%s.txt', experimento, experimento, sjnum);

practice_schedule_path = sprintf('/home/coleta3/Timing_CR/%s', arquivo_org_pratica);

frq_feedback_path = sprintf('/home/coleta3/Timing_CR/%s', arquivo_frq_feedback);
...
if feedback_fixo == '5' || feedback_fixo == '6'
	...
	yok_feedback = dlmread(practice_schedule_path, '\t', [0 4 183 4]); % [linha_inicial coluna_inicial linha_final coluna_final] % O primeiro índice é igual a 0 (ex.: coluna 1 = 0)
	arquivo_sj_path = sprintf('/home/coleta3/Timing_CR/%s/%s_yoked_suj%s.txt', experimento, experimento, sjnum);
end
...
transfer_vels_path = sprintf('/home/coleta3/Timing_CR/%s', arquivo_velocidades_transfer);
...
if pratica == 'det'
    practice_schedule = dlmread(practice_schedule_path, '\t', [0 0 183 7]);
...
HideCursor(wPtr);
% Set mouse on the stimuli screen so that it does not get in the way when
% The experimenter tries to insert the error estimation
SetMouse(rect(RectRight)/2, rect(RectBottom)/2, screenNum);
...
while trial < ntt
    ...
    if (questionario == '1') && (pratica == 'rtc' || pratica == 'trf') && trial == 1
		...
		if trial < ntt
			...
			while response_segunda_tela_quest(1) == 0
				...
				Screen('TextFont',wPtr, 'Ubuntu');
				Screen('TextSize',wPtr, 30);
				Screen('DrawText', wPtr, 'Pressione o botão do switch para iniciar a prática', (rect(RectLeft) + (referencia_larg*4)), 300, [255, 255, 255]);
    ...
    elseif pratica == 'trf' && trial > 1
        Screen('TextFont',wPtr, 'Ubuntu');
        Screen('TextSize',wPtr, 40);
        Screen('DrawText', wPtr, 'Pressione Enter para continuar ', rect(RectRight)/4.6, rect(RectBottom)/2.5, [255, 255, 255]);
        Screen('Flip', wPtr); %flip
        keyIsDown = 0;
        while keyIsDown == 0
            [keyIsDown, secs, keyCode, deltaSecs] = KbCheck;
  ...
  if ponto_para_oclusao == 0
    ...
    % Para rodar a animação sem a presença do mouse na tela
    SetMouse(rect(RectRight)/2, rect(RectBottom)/2, screenNum);
    ...
    if pratica == 'trf' || pratica == 'rtc'
        ...
        Screen('TextFont',wPtr, 'Ubuntu');
		Screen('TextSize',wPtr, 30);
		Screen('DrawText', wPtr, 'Estimativa...', rect(RectRight)/4.6, rect(RectBottom)/3, [255, 255, 255]);
		
		Screen('Flip', wPtr); % Flip
        
        
        % Get error estimation from participant (present a popup on screen 0)
		prompt = {'Estimativa de erro:'};
		dlg_title = 'Estimativa';
		num_lines = 1;
		error_estimation_tmp = inputdlg(prompt,dlg_title,num_lines);
		error_estimation = str2num(error_estimation_tmp{:});
		

	elseif (pratica == 'det') && (feedback_fixo == '4') && (trial <= ntt_baseline) % Vinte e três primeiras tentativas são de pre-teste
    
		% Determine an 'error estimation' value for trials in baseline
		error_estimation = 99999;
    
    elseif (pratica == 'det') && (feedback_fixo == '4') && (trial > ntt_baseline) % Vinte e três primeiras tentativas são de pre-teste
        
     
        
        if estimativa_durante_aq == '1'
		...
		elseif estimativa_durante_aq == '2'
			
			Screen('TextFont',wPtr, 'Ubuntu');
			Screen('TextSize',wPtr, 30);
			Screen('DrawText', wPtr, 'Por favor aguarde...', rect(RectRight)/4.6, rect(RectBottom)/3, [255, 255, 255]);
		...
		% Wait so that the button is not pressed when the feedback is presented
		WaitSecs(0.3);
    ...
    elseif (pratica == 'det') && (feedback_fixo == '5') && (trial == 3) % Três primeiras tentativas são de pre-teste
    ...
    elseif (pratica == 'det') && (isempty(feedback_fixo) || feedback_fixo == '1' || feedback_fixo == '2') %(feedback_fixo != '3') && (feedback_fixo != '4')
        
        
        if trial == 6 || trial == 9 || trial == 12 || trial == 15 || trial == 18 || trial == 21 ||...
                trial == 24 || trial == 27 || trial == 30 || trial == 33 || trial == 36 || trial == 39 || trial == 42 || trial == 45 || trial == 48 ||...
                trial == 51 || trial == 54 || trial == 57 || trial == 60 || trial == 63 || trial == 66 || trial == 69 || trial == 72 || trial == 75 ||...
                trial == 78 || trial == 81 || trial == 84 || trial == 87 || trial == 90 || trial == 93
	...
	elseif (pratica == 'det') && (feedback_fixo == '6') && (yok_feedback(trial) == 0)
		% Determine an 'error estimation' value for trials in baseline
		error_estimation = 99999;
	...
	elseif (pratica == 'det') && (feedback_fixo == '6') && (yok_feedback(trial) == 1)
		...
		%Screen('TextFont',wPtr, 'Ubuntu');
		%Screen('TextSize',wPtr, 30);
		%Screen('DrawText', wPtr, 'Pressione o mouse para continuar', rect(RectRight)/4.6, rect(RectBottom)/2.5, [255, 255, 255]);
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 	...
 	if questionario == '1' && pratica != 'trf' && pratica != 'rtc'
        ...
        if trial == 3 || trial == 18 || trial == 33 || trial == 48 || trial == 63 || trial == 78 || trial == 93
			...
			while response_segunda_tela_quest(1) == 0
				...
				Screen('TextFont',wPtr, 'Ubuntu');
				Screen('TextSize',wPtr, 30);
				Screen('DrawText', wPtr, 'Pressione o botão do switch para reiniciar a prática', (rect(RectLeft) + (referencia_larg*4)), 300, [255, 255, 255]);
    ...
    elseif questionario == '2'
		dados_tentativa = [str2num(escolha) time_escolha time_diff time_feedback str2num(fb_choice) time_stim tempo_entre_inicio_e_oclusao error_estimation];
    end
...
% 18/01/2019
% Acrescentei uma linha que permite ao script identificar (para os grupos yoked)
% Quando se trata de baseline (yok_feedback(trial) == 0).
% Como nos experimentos anteriores não havia baseline (ou seja yok_feedback com valor 0)
% O script travava por não haver, nessa condição, um valor para error_estimation.
% A modificação (na data em que isto foi feito) ocorreu na linha 1678 do script.


% 04/12/2018
% Acrescentei a possibilidade de mostrar ou não a palavra "estimativa..." para o participante.
% Isso torna possível ter os grupos que estimam e não estimam.
% Sempre será apresentado o popup para registrar a estimativa. 
% O experimentador deve digitar o valor ou digitar 99999 (dependendo do grupo experimental - sendo 99999 para os grupos sem estimativa de erro).

% 23/10/2018
% Acrescentada tela "pressione Enter para continuar" (para dar tempo de fazer a estimativa de erro - experimento MT)
% Linha: 1532
% Modificado o baseline. No experimento do Ricardo foram 3 tentativas. MT vai usar 23tt.
